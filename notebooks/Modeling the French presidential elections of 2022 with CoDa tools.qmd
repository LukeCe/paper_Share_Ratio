...
This vignette accompanies our article "Share-ratio interpretations of compositional regression models", which is currently [available as a working paper](https://www.tse-fr.eu/publications/share-ratio-interpretations-compositional-regression-models).
It contains all the code necessary to reproduce the article's results and provides additional illustrations.
...
sre_matrix_plot <- function(sre_tab, psize = 5, bin_centers) {
  ...
  if (missing(bin_centers)) {
    p <- ggplot(sre_tab) +
      ...
      labs(x = "share ratio of Y", y = "share ratio of X", col = "share ratio\nelasticity", shape = "sign") +
      theme_bw()  +
  ...
  if (!missing(bin_centers)) {
    ...
    p <- ggplot(sre_tab) +
      ...
      labs(x = "share ratio of Y", y = "share ratio of X",
...
The compensating direction is particularly interesting for this purpose because we know that the changes in the Y composition are entirely explained by changes in the two compensating components of the X composition.
...
```{r}
B    <- coef(VOTE_model,"clr",separate = TRUE)[["ilr(PROFCAT)"]]
