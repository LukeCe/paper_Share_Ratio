...
paris_pred_gg <- rbind(
  ...
  cbind("DIR" = "Direction with origin", paris_pred2_gg))

hline <- data.frame(
  INT = c(paris_pred1[c("0","0"), "PRED_TURNOUT"],paris_pred2["101", "PRED_TURNOUT"]),
  DIR = c("Direction with vertex","Direction with origin", "Direction with origin"))
...
ggplot(paris_pred_gg[between(F_SHARE, pclim[1], pclim[2]),]) +
  ...
  scale_y_continuous(limits = pclim,labels = scales::percent) +
  ...
  labs(y = "share in professional category", x = "predicted turnout",
       col = "professional category", lty = "professional category") +
  facet_wrap("DIR")
```
...
paris_size_scenario_gg <- melt(
  ...
  id.vars = "REGISTERED",
  measure.vars = paste0("VOTE.",colnames(VOTE)))

ggplot(paris_size_scenario_gg) +
  geom_vline(xintercept = paris$REGISTERED) +
  geom_line(aes(x = REGISTERED, y = value, col = variable)) +
...
paris_pred1_gg <- melt(
  data = data.table(paris_pred1),
  id.vars = c("H_SEQ","F_SHARE","PRED_TURNOUT_LOGIT","PRED_TURNOUT"),
  measure.vars = c(paste0("PROFCAT.", colnames(PROFCAT)), paste0("VOTE.", colnames(VOTE))))
...
paris_pred_gg[,SAHRE_XY := str_remove(COMPO,"VOTE.|PROFCAT.")]
...
ggplot(paris_pred_gg[between(F_SHARE, pclim[1], pclim[2]),]) +
  geom_vline(aes(xintercept = INT), data = hline) +
  geom_line(aes(y = value, x = F_SHARE, col =variable, lty = variable)) +
  scale_y_continuous(labels = scales::percent) +
  scale_x_continuous(labels = scales::percent) +
  theme_bw() +
  labs(y = "share of votes (bottom) or professional category (top)", x = "share of professional category Knowlege",
